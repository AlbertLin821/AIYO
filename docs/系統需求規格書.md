...
- **首次處理**：取得完整字幕後，用於段落摘要、語意分段、景點抽取，產出並儲存時間戳、summary、tags、embedding。處理完即釋放字幕，不持久化。
- **後續查詢**：不論同一使用者再次訪問或他使用者搜尋同類型影片，皆直接從 DB 讀取已儲存的時間戳與段落摘要，不重跑 Whisper 或重新分段，以節省時間與資源。
...
**資料儲存原則**：不儲存 YouTube 影片檔案或完整字幕原文，僅儲存影片 metadata、片段時間戳、tag 內容、summary 與 embedding 向量。

**資料表結構**：

1. **`videos`**：影片 metadata（youtube_id、title、channel、duration 等）
2. **`segments`**：
   - `id, video_id, start_sec, end_sec, summary, tags, embedding_vector`
   - `tags`：JSON 格式，含景點名稱、類型（美食/親子/室內/室外等）
3. **`places`**：
...
**查詢來源**：RAG 搜尋直接使用已索引的 segments（時間戳、summary、tags、embedding），不重新處理影片或字幕。
...
- 將 retrieve 出來的 segments（包含 summary、tags、place info）傳給 LLM
- 要求：
  - 整理成 3–5 個最推薦的片段
